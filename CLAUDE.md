...
# 선택적 설정 (기본값 있음)
TELEGRAM_BOT_TOKEN=                  # 비어있으면 텔레그램 알림 비활성화
...
### 사업체 (Users)
- `GET /api/v1/users` - 전체 사업체 목록
- `POST /api/v1/users` - 사업체 등록
- `GET /api/v1/users/{user_id}` - 사업체 단건 조회
- `PUT /api/v1/users/{id}` - 사업체 수정 (naver_store_name, crawl_interval_min 설정)
- `DELETE /api/v1/users/{user_id}` - 사업체 삭제
- `POST /api/v1/users/{user_id}/verify-password` - 비밀번호 검증
- `POST /api/v1/users/{user_id}/telegram-test` - 텔레그램 테스트 메시지 전송
...
- `GET /api/v1/products/{id}/shipping-overrides` - 배송비 오버라이드 목록
- `POST /api/v1/products/{id}/shipping-overrides` - 배송비 오버라이드 추가
- `PATCH /api/v1/products/{id}/shipping-overrides/{naver_product_id}` - 배송비 오버라이드 수정
- `DELETE /api/v1/products/{id}/shipping-overrides/{naver_product_id}` - 배송비 오버라이드 해제

### 키워드 (Keywords)
- `GET /api/v1/products/{product_id}/keywords` - 키워드 목록 조회
- `POST /api/v1/products/{product_id}/keywords` - 키워드 추가
- `DELETE /api/v1/keywords/{keyword_id}` - 키워드 삭제
- `POST /api/v1/keywords/suggest` - SEO 기반 키워드 추천

### 크롤링 (Crawl)
- `POST /api/v1/crawl/product/{id}` - 상품 크롤링 실행
- `POST /api/v1/crawl/user/{user_id}` - 유저 전체 크롤링
- `GET /api/v1/crawl/status/{user_id}` - 크롤링 상태
- `GET /api/v1/crawl/logs/{user_id}` - 크롤링 로그

### 비용/마진 (Costs & Margin)
- `GET /api/v1/products/{product_id}/costs` - 비용 항목 조회
- `PUT /api/v1/products/{product_id}/costs` - 비용 항목 저장
- `GET /api/v1/products/{product_id}/margin` - 마진 조회
- `POST /api/v1/products/{product_id}/margin/simulate` - 마진 시뮬레이션
- `GET /api/v1/users/{user_id}/cost-presets` - 프리셋 목록
- `POST /api/v1/users/{user_id}/cost-presets` - 프리셋 생성
- `PUT /api/v1/cost-presets/{id}` - 비용 프리셋 수정
- `DELETE /api/v1/cost-presets/{preset_id}` - 프리셋 삭제
- `POST /api/v1/cost-presets/{id}/apply` - 복수 상품에 프리셋 중첩 적용
- `POST /api/v1/cost-presets/{id}/detach` - 프리셋 해제 (해당 항목만 삭제)

### 가격 (Prices)
- `GET /api/v1/products/{product_id}/price-history` - 가격 히스토리
- `GET /api/v1/products/{product_id}/price-snapshot` - 가격 스냅샷

### 대시보드 (Dashboard)
- `GET /api/v1/users/{user_id}/dashboard/summary` - 대시보드 요약
- `GET /api/v1/users/{user_id}/dashboard/export` - CSV 내보내기

### 알림 (Alerts)
- `GET /api/v1/users/{user_id}/alerts` - 알림 목록
- `PATCH /api/v1/alerts/{alert_id}/read` - 알림 읽음 처리
- `POST /api/v1/users/{user_id}/alerts/read-all` - 전체 읽음
- `GET /api/v1/users/{user_id}/alert-settings` - 알림 설정 조회
- `PUT /api/v1/users/{user_id}/alert-settings` - 알림 설정 수정
...
28. 경쟁사 배송비 수동 입력 (shipping_overrides 테이블, CRUD API, 크롤링 통합, 즉시 반영)
29. 내 스토어 다른 상품 제외 허용 (모니터링 상품 자체만 보호, 동일 스토어 다른 상품은 블랙리스트/배송비 오버라이드 가능)
30. API Rate Limiting (크롤링 API 분당 10회 제한, slowapi)
31. 선택적 API Key 인증 (X-API-Key 헤더, 미설정 시 비활성화)
32. SSRF 방어 (배송비 스크래핑 리다이렉트 후 호스트 재검증)
33. 사업체별 비밀번호 인증 (bcrypt 해싱, 선택적 비밀번호 설정/변경/해제)
34. 텔레그램 봇 알림 (TELEGRAM_BOT_TOKEN 설정 시 크롤링 알림을 텔레그램으로 전송)
...
| `API_KEY` | "" | API 인증키 (빈 문자열이면 인증 비활성화) |
| `TELEGRAM_BOT_TOKEN` | "" | 텔레그램 봇 토큰 (빈 문자열이면 비활성화) |

## AI 에이전트 협업 가이드

### 역할 분담
- **Claude Code**: `backend/` + `frontend/` 전체 담당. 팀 모드로 백엔드/프론트엔드 병렬 개발.

### 작업 동기화 규칙
- **API 명세 기록**: 백엔드 API를 추가/수정하면, 반드시 이 문서(CLAUDE.md)의 API 엔드포인트 섹션에 변경된 Request/Response 구조를 명확히 기록한다.
...
### 2026-02-27: 경쟁사 배송비 수동 입력 (Shipping Override)

**새 테이블: `shipping_overrides`**
- `id` (PK), `product_id` (FK → products.id, CASCADE), `naver_product_id` (VARCHAR(50))
- `shipping_fee` (INTEGER, NOT NULL): 수동 입력 배송비
- `naver_product_name` (VARCHAR(500), nullable), `mall_name` (VARCHAR(200), nullable), `created_at`
- Unique constraint: `(product_id, naver_product_id)`

**새 API:**
- `GET /api/v1/products/{product_id}/shipping-overrides` — 배송비 오버라이드 목록 조회
  - Response: `[{id, naver_product_id, shipping_fee, naver_product_name, mall_name, created_at}]`

- `POST /api/v1/products/{product_id}/shipping-overrides` — 배송비 오버라이드 추가
  - Request: `{ naver_product_id: str, shipping_fee: int(0~100000), naver_product_name?: str, mall_name?: str }`
  - Response: `{id, naver_product_id, shipping_fee, naver_product_name, mall_name, created_at}` (201)
  - 중복 시 409, 즉시 반영: 해당 naver_product_id의 기존 rankings → shipping_fee/type 즉시 UPDATE

- `PATCH /api/v1/products/{product_id}/shipping-overrides/{naver_product_id}` — 배송비 금액 수정
  - Request: `{ shipping_fee: int(0~100000) }`
  - Response: 수정된 오버라이드 객체
  - 즉시 반영: 기존 rankings → shipping_fee/type 즉시 UPDATE

- `DELETE /api/v1/products/{product_id}/shipping-overrides/{naver_product_id}` — 배송비 오버라이드 해제
  - 204, 기존 rankings는 다음 크롤링에서 원본 배송비로 자동 복원

**스키마 변경:**
- `RankingItemResponse`에 `is_shipping_override: bool = False` 필드 추가
- `CompetitorSummary`에 `is_shipping_override: bool = False` 필드 추가
- 상품 상세 API keywords[].rankings[] 및 competitors[]에 `is_shipping_override` 필드 포함

**크롤링 변경:**
- `_save_keyword_result`에 `shipping_override_map` 파라미터 추가
- 오버라이드가 있는 naver_product_id는 `shipping_fee=오버라이드값`, `shipping_fee_type="paid"`로 대체
- `_crawl_product_impl`: 단일 상품 크롤링 시 오버라이드 맵 조회 후 전달
- `_crawl_user_all_impl`: 배치 쿼리로 상품별 오버라이드 맵 조회 후 전달
- 원본 배송비 스크래핑은 계속 수행 (오버라이드 해제 시 원본 사용 가능)

**즉시 반영 정책:**
- override 추가/수정 시: 해당 naver_product_id의 기존 rankings → shipping_fee 즉시 UPDATE
- override 삭제 시: 다음 크롤링에서 원본 배송비로 자동 복원 (즉시 반영 없음)

**새 파일:**
- `backend/app/models/shipping_override.py` — ShippingOverride 모델
- `backend/app/schemas/shipping_override.py` — Request/Update/Response 스키마
- `backend/app/services/shipping_override_service.py` — CRUD + 즉시 반영 서비스
- `backend/app/api/shipping_overrides.py` — REST API 엔드포인트

### 2026-03-04: selling_price 자동 갱신 버그 수정

**크롤링 변경 (manager.py):**
- 기존: `is_my_product` (유저의 모든 등록 상품) 기준으로 `selling_price` 자동 갱신 → 상품 A 크롤링 중 검색 결과에 상품 B(같은 유저)가 나오면 상품 A 가격이 상품 B 가격으로 덮어쓰기
- 수정: `is_this_product` (현재 크롤링 중인 상품의 `naver_product_id`와 일치하는 결과만) 기준으로 갱신
- `naver_product_id`가 미설정된 상품은 자동 갱신 대상에서 제외 (올바른 동작)

### 2026-03-04: 내 스토어 다른 상품 제외 허용

**기존 동작:** `is_my_store=true`인 모든 상품에 대해 제외(Ban)/배송비 오버라이드/수동 포함 예외 UI가 숨겨짐
**변경:** 현재 모니터링 중인 상품 자체(`myProductNaverId` 일치)만 보호, 같은 스토어의 다른 상품은 모든 기능 사용 가능

**프론트엔드 변경 (KeywordRankingList.tsx):**
- `myProductNaverId` prop 추가 (상품 상세 페이지에서 `product.naver_product_id` 전달)
- `isMyExactProduct()` 헬퍼 함수 추가: `is_my_store && naver_product_id === myProductNaverId`
- 7개 지점의 `!item.is_my_store` 조건을 `!isMyExactProduct(item)`으로 변경:
  - `canBan` (모바일 스와이프 제외 버튼)
  - 데스크톱 제외 버튼
  - `excludeImpactPreview` (영향도 미리보기)
  - `showIncludeControls` (수동 포함 예외 컨트롤)
  - `canShowShippingButton` (배송비 수동 입력 버튼)
  - `inferFilterReasons` (필터 사유 추론)
  - `getRelevanceReasonLabels` (관련성 사유 라벨)
- 내 스토어 다른 상품의 사유 라벨: `"내 스토어 (다른 상품)"`, `"수동 제외"` 표시

### 2026-03-05: 사업체별 비밀번호 인증

**User 모델 확장:**
- `password_hash` (VARCHAR(200), nullable): bcrypt 해시 저장

**스키마 변경:**
- `UserCreate`에 `password: str | None` 필드 추가 (4~100자, 선택)
- `UserUpdate`에 `password: str | None`, `remove_password: bool = False` 필드 추가
- `UserResponse`에 `has_password: bool` 필드 추가 (password_hash 존재 여부)
- `PasswordVerifyRequest`, `PasswordVerifyResponse` 신규 스키마

**새 API:**
- `POST /api/v1/users/{user_id}/verify-password` — 비밀번호 검증
  - Request: `{ password: str }`
  - Response: `{ verified: bool }`
  - 비밀번호 미설정 사업체는 항상 `verified: true`

**기존 API 변경:**
- `POST /api/v1/users`: `password` 필드 선택 입력 시 bcrypt 해싱 후 저장
- `PUT /api/v1/users/{id}`: `password` 필드로 비밀번호 변경, `remove_password: true`로 해제

**프론트엔드 변경:**
- `Header.tsx`: 비밀번호 있는 사업체 선택 시 인증 모달 표시, 등록 폼에 비밀번호 필드 추가, 자동선택은 비밀번호 없는 사업체만
- `settings/platforms/page.tsx`: 비밀번호 설정/변경/해제 섹션 추가

**하위호환:**
- `password_hash` nullable → 기존 사업체 영향 없음
- 비밀번호 없는 사업체는 기존과 동일하게 바로 접근 가능

### 2026-03-06: 텔레그램 봇 알림

**User 모델 확장:**
- `telegram_chat_id` (VARCHAR(50), nullable): 텔레그램 Chat ID

**스키마 변경:**
- `UserUpdate`에 `telegram_chat_id: str | None` 필드 추가
- `UserResponse`에 `telegram_chat_id: str | None` 필드 추가

**새 API:**
- `POST /api/v1/users/{user_id}/telegram-test` — 텔레그램 테스트 메시지 전송
  - Response: `{ ok: true }`
  - chat_id 미설정 또는 TELEGRAM_BOT_TOKEN 미설정 시 400

**기존 API 변경:**
- `PUT /api/v1/users/{id}`: `telegram_chat_id` 필드로 Chat ID 설정/해제 (빈 문자열 = 해제)

**알림 연동:**
- `alert_service.py`: 최저가 이탈, 순위 하락 알림 생성 시 텔레그램 자동 전송
- 웹 푸시와 독립적으로 동작 (둘 다 활성화 가능)

**새 파일:**
- `backend/app/services/telegram_service.py` — 텔레그램 Bot API 전송 서비스

**프론트엔드 변경:**
- `settings/platforms/page.tsx`: 텔레그램 Chat ID 입력/저장/테스트 섹션 추가

**하위호환:**
- `telegram_chat_id` nullable → 기존 사업체 영향 없음
- `TELEGRAM_BOT_TOKEN` 미설정 시 텔레그램 비활성화
